...
def get_keys_from_html(soup_html):
    return [tag['data-webtasks-id'] for tag in soup_html.find_all(lambda t: t.has_attr('data-webtasks-id'))]

def get_key_containing_target_key(highest_keys, target_key, soup, accuracy=1.0):

    if accuracy < 1.0:
        r = random.random()
        if r > accuracy:
            return random.choice(highest_keys)

    # find all element keys in html
    for h_key in highest_keys:
        e_html = get_element_html(h_key, soup)
        e_keys = get_keys_from_html(e_html)
        if target_key in e_keys:
            return h_key
        
    print(f'warn: target key {target_key} not found')
    return None#random.choice(highest_keys)


def get_element_html(key, soup):
    return soup.find(attrs={"data-webtasks-id": key})

def filter_duplicates_and_parents(keys, soup):
    # filer duplicates and elements that contain other elements
    e_html = [get_element_html(k, soup) for k in keys]
    ...
    for k, v in zip(keys, e_html):
        ...
        if not v in seen_vals or v is None:
            ...
            for o_val in e_html:
                o_val = str(o_val)
                if len(v) > len(o_val) and o_val in v:
                    # don't take these
                    is_smallest = False
...
def find_child_keys(element, depth_idx, highest_keys, max_depth):
    if depth_idx >= max_depth:
        return
    for c in element.children:
        if not isinstance(c, bs4.element.NavigableString):
            highest_keys.append(c.get("data-webtasks-id"))
            find_child_keys(c, depth_idx+1, highest_keys, max_depth)


def get_highest_keys(soup, bboxes_keys, max_depth=5):

    highest_keys = []
    find_child_keys(soup, 0, highest_keys, max_depth)

    highest_keys = [k for k in highest_keys if k in bboxes_keys]
    highest_keys = np.unique(highest_keys).tolist()
    highest_keys = filter_duplicates_and_parents(highest_keys, soup)
    return highest_keys
...
def build_records_for_single_turn(
    ...
    bboxes_keys = [e.attrib[uid_key] for e in elements_filt]
    soup = BeautifulSoup(turn.html, 'html.parser')

    highest_keys = get_highest_keys(soup, bboxes_keys)

    accuracy = 1.0

    if len(highest_keys) > 0:
        # print("highest keys found")
        # pick index that contains desired key
        chosen_key = get_key_containing_target_key(highest_keys, target_uid, soup, accuracy=accuracy)
        if chosen_key is not None:
            area_html = get_element_html(chosen_key, soup)
        else:
            area_html = soup
    else:
        print("warn: no highest keys found")
        area_html = soup

    small_keys = get_elements_with_n_childs(area_html, 1)
